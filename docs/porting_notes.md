...
> 60 秒・233 秒で測り直した結果は下の節にあります。**漏れは残っています**（233 秒で腰ベルト 1.44 倍）が、
> 破綻の桁ではありません。

## `JointVelocityIterations` の見直し（2026-09-05）— **据え置き**

パリティ更新で鎖の発散の原因側が入ったので、対症療法だった `JointVelocityIterations = 40` を
下げられるか 3 モデルで測り直しました。**下げられません。40 と `JointMaxCorrectionVel = 30` は据え置きです。**

測り方: `MmdPhysics.Editor.ChainStability` に `MMD_CHAIN_SOLVER="jointiter=N,maxcorr=M"` と
`MMD_CHAIN_SECONDS` を与える。鎖の伸びと貫入を**同じ 1 回の再生**から出す（貫入の集計は今回追加した）。

| 反復 | 13 節しっぽ・233 秒 | 13 節しっぽ・60 秒 | 髪の鎖・60 秒 | スカート・60 秒 |
|---|---|---|---|---|
| 10 | — | **343 倍** | **7.31 倍** | **3.16 倍** |
| 12 | — | **336 倍** | — | — |
| 20 | — | **325 倍** | 1.32 倍 | — |
| 24 | **338 倍** | 1.24 倍 | — | — |
| 32 | 1.44 倍 | 1.27 倍 | — | — |
| 40 | 1.44 倍 | 1.29 倍 | 1.28 倍 | 1.89 倍 |
| 60 | — | 1.24 倍 | 1.28 倍 | — |

- **60 秒では 24 が通ってしまい、233 秒で破綻します。** 短い再生でこの値を決めてはいけません。
- 崖は 24 と 32 の間（曲の全長で見たとき）。40 は 1.25〜1.7 倍の余裕。

**下げる理由だった「貫入が増える」は再現しませんでした。** 深貫入（>0.5 PMX 単位）の割合は反復にほぼ依存しません。

| 反復 | 13 節しっぽ・60 秒 | 髪の鎖・60 秒 | スカート・60 秒 |
|---|---|---|---|
| 10 | 0.359% | 2 件 | 11 件 |
| 20 | 0.257% | **0 件** | — |
| 40 | 0.281% | **0 件** | 18 件 |
...
移植元で観測された「40 で深貫入 0 → 643 件」は**接触編のパリティ更新前**の値で、いまの接触
（Bullet のマニフォールド管理・受理閾値・摩擦整列）では起きません。

`JointMaxCorrectionVel` も同様に測り直しました。「100 は 233 秒で 468 倍で破綻」は**もう再現しません**
（100 でも 1.44 倍で通る）。それでも 30 を選ぶのは 233 秒の実測が一番良いからです
（上限 10: 鎖 1.47 / 深貫入 4.168%、**30: 1.44 / 3.618%**、100: 1.44 / 3.673%）。

> **別件として残る観測**: 13 節しっぽのモデルは 233 秒で深貫入が **3.6〜4.4%**（60 秒では 0.3%）あり、
> どの設定でも変わりません。曲の後半で腕が髪を通るためで、この反復とは無関係の別課題です。
