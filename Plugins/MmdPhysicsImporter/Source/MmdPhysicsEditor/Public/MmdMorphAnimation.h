...
//
// ★飛ばすだけでは足りない場合がある。**取り込みの時点で既に化けたカーブが
//   入っていることがある**ので、それは消す。
//
//   Interchange が `weights` を取り込めた `.glb` (Tda式 V4X で確認) では、
//   AnimSequence に記号モーフのカーブも作られる。UE は上記のとおり
//   RegenerateLegacyCurveData() でカーブ一覧を**リグ側の名前から作り直す**ので、
//   出来上がるのは `▲` `∧` `□` が 1 本に潰れた `_`、`ω□` の `ω_`、
//   `恐ろしい子！` の `恐ろしい子_` といった名前になる。
//   これらは **どのモーフターゲットにも対応せず、スケルトンにも登録されていない**。
//   プラグインは「既存のカーブは触らない」ので、放っておくと残り続ける
//   (実際に MmdPhysics.Editor.BuildActor が「カーブ名がすべてモーフターゲットに
//    対応している」で Fail する)。
//
//   消す条件は 2 つ揃ったときだけにする:
//     1. `Mesh->FindMorphTarget(カーブ名) == nullptr` (何も動かさないカーブ)
//     2. カーブ名が「サニタイズで名前が変わるモーフ」のサニタイズ像と一致する
//        (= 上の化け方で説明が付く)
//   片方だけなら消さずに Warning に出す。1 だけで消すと、事情の分からない
//   壊れたカーブまで黙って消すことになり、2 だけなら化けていない実在のモーフを消す。
//
//   ★カーブ名だけでは判定できない。化けた名前 (`_`、`ω_`) は**既にリグ規則に
//     載っている**ので MakeRigSafeName を通しても変わらない。飛ばしたモーフ名から
//     サニタイズ像を作って突き合わせる必要がある (CollectRigMangledNames)。
//
//   スケルトンの curve metadata は触らない (他のアセットと共有しているため)。
// ===========================================================================
...
struct MMDPHYSICSEDITOR_API FMmdMorphAnimResult
{
	...
	/**
	 * 取り込みの時点で化けていたので消したカーブの数 (上の注記を参照)。
	 * ★0 でなくても **アニメーションのアセットを保存しないと元に戻る**。
	 *   呼び出し側 (FMmdActorBuilder) が保存する。
	 */
	int32 CurvesRemoved = 0;
	/** 上で消したカーブ名。ログとメッセージに出す。 */
	TArray<FString> RemovedNames;
	/**
	 * モーフターゲットに対応しないが、化け方の説明が付かないので**残した**カーブ名。
	 * 消す条件の片方しか満たさないもの。Warning に出すだけで触らない。
	 */
	TArray<FString> UnmatchedKeptNames;
	/**
...
public:
	...
	/**
	 * モーフ名の一覧から「カーブにできない名前が UE に化けさせられた姿」を集める。
	 *
	 * 返るのは `MakeRigSafeName(名前) != 名前` になるモーフだけを通した像で、
	 * `▲` `∧` `□` → `_` のように**複数のモーフが 1 つに潰れる**ため
	 * 元の名前より少なくなる。ApplyMorphCurves はこの集合に載っていて、かつ
	 * メッシュのモーフターゲットに無いカーブだけを消す (上の注記を参照)。
	 */
	static TSet<FName> CollectRigMangledNames(const TArray<FString>& MorphNames);
};
