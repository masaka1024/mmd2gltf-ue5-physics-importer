...
struct MMDPHYSICSRUNTIME_API FAnimNode_MmdPhysics : public FAnimNode_SkeletalControlBase
{
	 ...
	 *
	 * ★2026-09-05 再測定 (エンジンのパリティ更新後・3 モデル)。**40 のまま据え置くと決めた。**
	 *   パリティ更新で鎖の発散の原因側 (腕長ゲート / 減衰クランプ / ばねのモーター行化) が
	 *   入ったので下げられるか測り直したが、**下げられない**:
	 *
	 *     13 節しっぽのモデル・233 秒 … 24 は しっぽ３ **338 倍**で破綻 / 32 と 40 は 1.44 倍で通る
	 *     同・60 秒                   … 10/12/20 が 325〜343 倍で破綻 / 24 以上は 1.2 倍台
	 *                                   (★60 秒では 24 が通ってしまう。短い再生で決めてはいけない)
	 *     髪の鎖のモデル・60 秒       … 10 で もみあげ 7.31 倍 / 20 以上は 1.3 倍台
	 *     スカートのモデル・60 秒     … 10 で スカート 3.16 倍 / 40 は 1.89 倍
	 *
	 * ★**下げる理由だった「貫入が増える」は再現しなかった。** 同じ再生から数えた深貫入
	 *   (>0.5 PMX単位) は反復にほぼ依存しない:
	 *     13 節しっぽ・60 秒 … 反復 10:0.359% / 20:0.257% / 24:0.308% / 40:0.281% / 60:0.232%
	 *     髪の鎖・60 秒      … 反復 20/40/60 とも **0 件**、10 のときだけ 2 件
	 *     スカート・60 秒    … 反復 10:11 件 / 40:18 件 (どちらも接触 38 万件に対して)
	 *   移植元で観測された「40 で深貫入 0→643 件」は接触編のパリティ更新前の値で、
	 *   いまの接触 (Bullet のマニフォールド管理・受理閾値・摩擦整列) では起きない。
	 *
	 * ★接触 (SolverIterations) は 10 のまま据え置くこと。接触の反復を増やすと
	 *   待機区間の揺れが悪化する (実測 8.91cm/1.72cm、既定 7.17/0.96 より悪い)。
	 ...
	 *
	 * ★2026-09-05 再測定 (パリティ更新後)。**30 のまま据え置くと決めた。**
	 *   上の「100 が 233 秒で 468 倍」は**もう再現しない** (100 でも 1.44 倍で通る)。
	 *   原因側が入ったので、この上限はもはや破綻を止める役ではなくなっている。
	 *   それでも 30 を選ぶのは、233 秒で計った値が 30 のとき一番良いから:
	 *     上限 10  … 鎖 1.47 倍 / 深貫入 4.168% / 最深 -1.750
	 *     上限 30  … 鎖 1.44 倍 / 深貫入 3.618% / 最深 -1.590   ← 採用
	 *     上限 100 … 鎖 1.44 倍 / 深貫入 3.673% / 最深 -1.572
	 *   差は小さい。**コア既定 (10) へ寄せる意味はあるが、数値は悪くなる**ので動かさない。
	 */
