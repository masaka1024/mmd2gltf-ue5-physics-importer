...
namespace MmdPhysics
{
	...
	public:
		...
		/**
		 * 速度求解でジョイントだけを追加で回す回数。0 = SolverIterations と同じ (従来の挙動)。
		 *
		 * ★長い鎖が伸び続けるのを止めるための値。
		 *   Gauss-Seidel は 1 反復あたりジョイント 1 本ぶんしか情報を伝えないので、
		 *   反復数より節数が多い鎖ではアンカー側の拘束が末端まで届かない。
		 *   届かなかったぶんの相対速度が毎サブステップ残り、位置誤差として**単調に溜まる**
		 *   (戻らないラチェットになる。位置補正 (Baumgarte / split-impulse) を強めても
		 *   直らないことは実測済み)。
		 *   実測 (Tda式初音ミク・アペンド Ver1.10 の しっぽ = 13 節、アニメ再生):
		 *     10 (=従来) … アンカー差が 0.01→0.05→0.15cm と毎フレーム増え一度も戻らない。
		 *                   20 秒で しっぽ３ が参照ポーズの 220 倍、鎖が真下へ 55cm/秒 で落ち続ける
		 *     20         … 20 秒では収まるが 60 秒で再発 (123 倍)
		 *     40 (+JointMaxCorrectionVel 30) … 233 秒 (曲の全長) でも収まる
		 *   ★位置補正側の反復を増やしても効かない (40 回にしても 178 倍)。効くのは速度側だけ。
		 *
		 * ★接触と分けてある理由: 接触の反復数を増やすと待機区間の揺れが**悪化**する
		 *   (IA 実測: 反復増で 8.91cm/1.72cm と、既定 7.17/0.96 より悪い)。
		 *   鎖に要るのはジョイントの伝播だけなので、接触は SolverIterations のまま据え置く。
		 *   衝突検出とばねはサブステップに 1 回きりで、ここで増えるのは行の解き直しだけ。
		 *
		 * ★コア既定は 0 (=従来と完全に同じ) のまま。数値パリティテストは移植元 C# と
		 *   ビット一致で比べるため。実際に再生する値は FAnimNode_MmdPhysics 側が持つ
		 *   (docs/porting_notes.md「ソルバ既定値はノードとコアで違う」)。
		 */
		int32 JointVelocityIterations = 0;

		/**
		 * ジョイントの位置補正 (Baumgarte) 速度の上限。0 = Joint::MaxCorrectionVel を触らない
		 * (= 従来どおり 10)。
		 *
		 * ★JointVelocityIterations と**対で**使う。片方だけでは直らない:
		 *   - 反復だけ増やす … 破綻を遅らせるだけ。jointIter=40 でも 120 秒で 201 倍まで戻る
		 *   - 上限だけ上げる … 速度が収束していない状態で補正を強めると**悪化**する
		 *                      (jointIter=10 のまま無制限にすると 220 倍 → 426 倍)
		 *   - 両方           … 233 秒 (曲の全長) でも収まる (しっぽが結果から消える)
		 *
		 * ★機構: 速度求解の解き残しが位置誤差として毎サブステップ積もる。誤差が
		 *   10 / (Beta=0.2 * 1/dt) ≒ 0.42 PMX単位 (3.3cm) を超えると補正速度が上限に張り付き、
		 *   重力で開く速さに追いつけなくなる。以後この誤差は**二度と戻らない**
		 *   (最小比がちょうど 1.00 のまま最大比だけ伸び続けるのはこのため)。
		 *   実測 (しっぽ 13 節、jointIter=40): 上限 10 は 120 秒で 201 倍。100 は 120 秒
		 *   までは収まるが **233 秒 (曲の全長) で 468 倍**。30 は 233 秒でも 1.20 倍で通る。
		 *   強い補正はアンカーの開きに比例した反動も強めるため、上限は
		 *   「発散を止められる範囲で一番弱い値」を選ぶ (低すぎても高すぎても壊れる)。
		 *
		 * ★コア既定は 0 (=従来と完全に同じ) のまま。パリティテストのビット一致を守るため。
		 *   再生時の値は FAnimNode_MmdPhysics 側が持つ。
		 */
		float JointMaxCorrectionVel = 0.0f;
