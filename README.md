...
- **アウトライン（エッジ）は未対応です。** `flags` の bit4（値 16）が輪郭線フラグで、
  `edgeColor` / `edgeSize` はマテリアルインスタンスのパラメータとして保存してあります。
  実装する場合は反転メッシュ法（法線方向へ押し出した表裏反転のスロットを追加する）になり、
  マテリアルだけでは足りず**メッシュのセクションを増やす処理**が要ります。太さは移植元と同じく
  `edgeSize × 係数`（移植元は lilToon の `_OutlineWidth` 既定 0.08 に合わせた暫定値）で換算します
