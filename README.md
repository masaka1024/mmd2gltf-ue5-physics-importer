...
| 半透明の材質がギザギザに切り抜かれる | その材質のマテリアルインスタンスの親が `M_MmdToonTranslucent` になっているか。なっていなければ `.glb` 側の `alphaMode` が `BLEND` でない可能性があります |
...
| | 状況 |
|---|---|
| UE 5.5 | 開発・検証環境。自動テスト 10 件が green |
...
- **半透明材質の内部の前後関係（移植元 lilToon の TwoPass 相当）は再現していません。** UE の
  半透明マテリアルには深度書き込みプリパスが無いためです。実害が出るのは
  「1 つの `alphaMode=BLEND` 材質が自分自身と重なる」場合（BLEND で作られた透け髪など）に限られます。
  MMD の髪・肌・服は通常 `MASK` なので不透明パスで描かれ、この問題は起きません
- **`extras.mmd` の `origTexture`（プリベイク前の無加工テクスチャ）は未対応です。**
  移植元はこれがある材質を `MASK` → 半透明へ昇格させて MMD 本来の柔らかい眉・透け髪を再現しますが、
  UE 版は GLB バイナリからのテクスチャ抽出を実装していないため、`alphaMode` をそのまま使います
