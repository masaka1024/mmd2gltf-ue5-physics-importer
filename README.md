...
5. 続けて **「3. アクターを生成 (本体 + 輪郭線)」**
6. できた `BP_<メッシュ名>` をレベルへ置いて再生

【1】を実行すると `ABP_<メッシュ名>_MmdPhysics` という Post-Process Anim Blueprint が
スケルタルメッシュの隣に作られ、割り当てられます。

【3】を実行すると `BP_<メッシュ名>` という Blueprint アクターができます。スケルタルメッシュと
輪郭線コンポーネント（`MmdOutlineComponent`）が組まれた状態なので、レベルへ置くだけで
物理も輪郭線も効きます。輪郭線を描く材質が無いモデルには輪郭線コンポーネントを付けません。
**【2】のあとに実行してください**（輪郭線の有無をマテリアルから判断するため）。
...
| | 状況 |
|---|---|
| UE 5.5 | 開発・検証環境。自動テスト 13 件が green |
